...
<head>
    ...
    <title>Recuperar Contraseña - AudioGestor</title>
...
<body class="bg-[#FDFDFC] dark:bg-[#0a0a0a] flex items-center justify-center min-h-screen">
    <div class="w-full max-w-md mx-4">
        ...
        <div class="bg-white dark:bg-[#161615] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-lg p-6">
            @if (session('status'))
                <div class="mb-4 p-3 bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 rounded-sm text-sm">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('password.email') }}">
                ...
                <!-- Botón de Envío -->
                <button type="submit" 
                        class="w-full bg-[#1b1b18] dark:bg-[#eeeeec] text-white dark:text-[#1C1C1A] py-2 px-4 rounded-sm font-medium hover:bg-black dark:hover:bg-white transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 mb-4">
